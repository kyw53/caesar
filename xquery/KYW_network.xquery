...
<html>
    ...
    <p>
        ...
        <p>This Kumu network diagram, similar to the one above, shows a distinct list of Romans for each book, divided into sections for Gaul and the Civil War, with the person nodes scaled to the number of appearances in a given book. However, it also uses a Field, called Name, to create connections for the names across <i>all</i> of the books. Now with this graph, we can see that a Roman like <a href="https://en.wikipedia.org/wiki/Titus_Labienus">Titus Labienus</a> was heavily mentioned in the Gallic Wars section, but not nearly as much in the latter. Then with research, we were able to uncover that Labienus seems to have dwindled in his support for Caesar, dying in resistance at <a href="https://en.wikipedia.org/wiki/Battle_of_Munda">Munda</a> in 45 BCE.</p>
        </div>
        </body>
        </html>